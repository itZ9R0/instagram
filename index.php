...
<html lang="en">
  ...
  <body>
		...
		<div class="row"> 
			...
			<div class="col-8 px-0 my-2"> 
				<?php
					...
					mysqli_query($con, "UPDATE posts SET text = 'Игра в баскетбол' WHERE id=2 ");
				?>
					...
					<div class="col-12 py-2">  <!--текстовые элементы поста-->
						...
						<div class="change">
							<form method="GET" action="update.php">
								<input class="" type="" name="id" value="<?php
									echo $stroka["id"];
								?>">
								<input class="" type="" name="text" value="<?php
									echo $stroka["img"];
								?>">
								<input class="" type="" name="text" value="<?php
									echo $stroka["text"];
						...
						<button class="changebut">Обновить</button>
						<div>
							<p>5 недель назад</p>
						</div>						
					</div>
	...
	<script type="text/javascript">
		let mm = document.querySelectorAll(".change")
		ATK = document.querySelectorAll(".changebut")
		
		for (let i=0;i<3;i++) {
			mm[i].style.display = "none"
			
			ATK[i].onclick = function(){
				mm[i].style.display = "block"
				ATK[i].style.display = "none"
			}
		}
